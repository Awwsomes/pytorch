import sys
import os
os.environ["QT_DEBUG_PLUGINS"] = "1" # 查看插件加载
import shutil
...
class ImageLabel(QLabel):
    ...
    def __init__(self, image_path, img_data, txt_dir=None):
        super().__init__()
        ...
        self.pixmap_original.loadFromData(img_data)
        t2 = time.time()
        self.update_display()
        t3 = time.time()
        self.load_img_time = t2 - t1
        self.update_display_time = t3 - t2
        self.scale_pixmap_time = 0
...
class DatasetSorter(QMainWindow):
    def __init__(self, root_dir:str, class_names:list, txt_dir:str):
        ...
        # 新增图像和标签打包成数据库
        self.db = sqlite3.connect(os.path.join(root_dir, "class_data.db"))  # 连接到数据库（如果文件不存在，会自动创建）
    ...
    def init_db(self):
        ...
        # 加载图像数据
        self.db.execute("PRAGMA journal_mode=WAL;")  # 开启 WAL 模式：读取和写入可以并行，极大地提升 UI 响应速度
        # 准备批量数据
        tasks = []
        for root, _, files in os.walk(self.root_dir):
            for f in files:
                if f.lower().endswith((".png", ".jpg", ".jpeg", ".bmp")):
                    ...
                    # 默认分类可以设为文件夹名或 "Unclassified"
                    category = os.path.basename(root)
                    tasks.append((full_path, category))
        # 批量插入：使用 BEGIN...COMMIT 包裹
        self.cursor.execute("BEGIN TRANSACTION;")
        self.cursor.executemany(
            "INSERT OR IGNORE INTO dataset (file_path, category) VALUES (?, ?)",
            tasks
        )
        # 找出还没读取图片的记录
        cursor1 = self.db.execute("SELECT id, file_path FROM dataset WHERE thumbnail IS NULL")
        rows = cursor1.fetchall()
        for row_id, path in rows:
            with open(path, 'rb') as img:
                blob = img.read()  # 和readlines有什么区别？
            if blob:
                self.db.execute("UPDATE dataset SET thumbnail = ? WHERE id = ?", (blob, row_id))
                self.db.commit()  # 或者每 50 张 commit 一次
        # print(1)
    ...
    def refresh_tab(self, index):
        ...
        time1 = time.time()
        cursor = self.db.execute("SELECT file_path,thumbnail FROM dataset WHERE category = ?",
        (current_class,))
        files = cursor.fetchall()
        time2 = time.time()
        print(f"io耗时: {time2 - time1 :.4f}")
        ...
        time3 = time.time()
        duration = 0
        read_img_data_dura = 0
        update_display_time = 0
        scale_time = 0
        for i, (file_path, binary_data) in enumerate(files):
            # print(i)
            try:
                # 【修改】传入 txt_dir 和 class_names
                t1 = time.time()
                img_widget = ImageLabel(file_path, binary_data, self.txt_dir)
                read_img_data_dura += img_widget.load_img_time
                update_display_time += img_widget.update_display_time
                scale_time += img_widget.scale_pixmap_time
                t2 = time.time()
                duration += t2 - t1
                ...
                pass
        time4 = time.time()
        print(f"构造总耗时: {time4 - time3:.4f}, imageLabel耗时: {duration:.4f}")

        print(f"img: {read_img_data_dura: .4f}, dispaly: {update_display_time:.4f}")
        print(f"scale: {scale_time:.4f}")
        # print(10)
    ...
    def undo_operation(self):
        ...
        # 逆向移动：从目标路径移回源路径
        for src_path, src_idx, target_idx in last_operation:
            self.update_category(self.class_names[src_idx - 1], src_path)
            undo_count += 1
    ...
    def redo_operation(self):
        ...
        # 重新执行移动：从源路径移到目标路径
        for src_path, src_idx, target_idx in last_operation:
            self.update_category(self.class_names[target_idx - 1], src_path)
            redo_count += 1
    ...
    def update_category(self, new_category, file_path):
        self.db.execute(
            "UPDATE dataset SET category = ? WHERE file_path = ?",
    ...
    def process_move(self, target_idx):
        ...
        for img_widget in images_to_move:
            ...
            cursor1 = self.db.execute("SELECT category FROM dataset WHERE file_path = ?",
                                      (src_path,))
            src_label = cursor1.fetchone()[0]
            src_idx = self.class_names.index(src_label) + 1

            print(3)

            if target_idx == len(self.class_names):
                self.update_category("垃圾桶", src_path)
            else:
                self.update_category(self.class_names[target_idx - 1], src_path)
            success_count += 1
            # 【新增】记录源路径和最终的目标路径
            current_operation.append((src_path, src_idx, target_idx))
        ...
        if success_count > 0:
            if target_idx == len(self.class_names):
                ...
                self.operation_label.setText(f"✅ 已批量移动 {success_count} 张至 [{target_idx}] {self.class_names[target_idx - 1]}")
