    ...
    for txt in list_txt:
        ...
        with open(path_txt,'r') as txt_file:
            ...
            for idx,line in enumerate(txt_file):
                # 找置信度最高的一行
                conv = float(line.split(' ')[5])
                if conv > max_conv:
                    max_conv = conv
                    label_max_conv = int(line.split(' ')[0])
            # 检查预测标签和实际标签是否一致
            if label_max_conv == label_predict:
...
if __name__ == "__main__":
    path_root = r"D:\A_myData\Pytorch\yolov5-master\runs\detect\exp58"  # 模型预测出来的标签文件夹
    path_output_txt = r"D:\A_myData\Pytorch\src\data\yolov5_gazebo_6000_exp58.txt"  # 存储输出数据的txt文件
