...
path_txt = r"D:\A_myData\Pytorch\src\data\yolov5_gazebo_6000_exp58.txt"   # 存放数据的txt
...
try:
            ...
            if len(parts) >= 3:
                ...
                try:
                    ...
                    if 0 <= label_int < 31:
                        ...
                        recalls[label_int] = float(recall_val)
                    else:
    ...
    recalls = np.random.uniform(0.5, 0.95, 31)
...
# 3. 绘制召回率（Recall）柱
# 绘制在 x_pos + offset 位置
bars_recall = ax.bar(x_pos + offset,
                     data['Recall'],
                     bar_width,
                     label='Recall',
                     color='#55A868')
...
# 召回率标签 (向上偏移 0.005)
add_labels(bars_recall, 0.005, '#55A868')
...
ax.legend()
# Y轴范围通常是 0 到 1
ax.set_ylim(0, data[['Precision', 'Recall']].values.max() * 1.05 + 0.05)
